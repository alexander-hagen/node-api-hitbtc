...
//      logger.warn('ws closed');
      ...
      this._promises.forEach((cb, id) => { this._promises.delete(id); });
...
//      setTimeout(() => this._createSocket(this._ws._url), 500);
  ...
  request(method, ch, params = {}) {
    if (this._ws.readyState === WebSocket.OPEN) {
      ...
      return Promise.reject({"code":"503","error":"Service Unavailable"})
    }
...
var HitbtcSocket = function(url, keys) {
  ...
  this._bucket={
    wsLogin: new TokenBucket({ bucketSize: 5, tokensPerInterval: 5, interval: "second" }),
    wsSpotSubscribe: new TokenBucket({ bucketSize: 5, tokensPerInterval: 5, interval: "second" }),
    wsMarginSubscribe: new TokenBucket({ bucketSize: 5, tokensPerInterval: 5, interval: "second" }),
    wsFuturesSubscribe: new TokenBucket({ bucketSize: 5, tokensPerInterval: 5, interval: "second" }),
    wsSpotBalance: new TokenBucket({ bucketSize: 30, tokensPerInterval: 20, interval: "second" }),
    wsMarginBalance: new TokenBucket({ bucketSize: 30, tokensPerInterval: 20, interval: "second" }),
    wsFuturesBalance: new TokenBucket({ bucketSize: 30, tokensPerInterval: 20, interval: "second" }),
    wsSpotOrder: new TokenBucket({ bucketSize: 500, tokensPerInterval: 300, interval: "second" }),
    wsMarginOrder: new TokenBucket({ bucketSize: 500, tokensPerInterval: 300, interval: "second" }),
    wsFuturesOrder: new TokenBucket({ bucketSize: 500, tokensPerInterval: 300, interval: "second" }),
    wsWalletSubscribe: new TokenBucket({ bucketSize: 15, tokensPerInterval: 5, interval: "second" }),
  };
...
HitbtcSocket.prototype.login = async function(ws, api) {
  ...
  const remaining=await this._bucket["wsLogin"].removeTokens(1);
  try {
    const
      window=60000,
      stamp=Date.now(),
      source=stamp.toString()+window.toString(),
      signature=crypto.createHmac('sha256',api.secret).update(source).digest('hex'),
      params = {
//        type: "Basic",
//        api_key: api.apikey,
//        secret_key: api.secret,
    ...
    const result=await ws.socket.request('login', undefined, params);
...
HitbtcSocket.prototype.subscribeTrades = async function(options) { // async
  const remaining=await bucket["wsPublic"].removeTokens(1);
...
HitbtcSocket.prototype.subscribeSpotReports = async function() { // async
  const remaining=await this._bucket["wsSpotSubscribe"].removeTokens(1);
...
HitbtcSocket.prototype.getSpotOrders = async function(options) { // async
  const remaining=await bucket["wsTrading"].removeTokens(1);
...
HitbtcSocket.prototype.createSpotOrder = async function(options) { // async
  const remaining=await this._bucket["wsSpotOrder"].removeTokens(1);
...
HitbtcSocket.prototype.getSpotBalances = async function() { // async
  const remaining=await this._bucket["wsSpotBalance"].removeTokens(1);
...
HitbtcSocket.prototype.subscribeMarginReports = async function() { // async
  const remaining=await this._bucket["wsMarginSubscribe"].removeTokens(1);
...
HitbtcSocket.prototype.createMarginOrder = async function(options) { // async
  const remaining=await this._bucket["wsMarginOrder"].removeTokens(1);
...
HitbtcSocket.prototype.getMarginAccounts = async function() { // async
  const remaining=await this._bucket["wsMarginBalance"].removeTokens(1);
...
HitbtcSocket.prototype.subscribeFuturesReports = async function() { // async
  const remaining=await this._bucket["wsFuturesSubscribe"].removeTokens(1);
...
HitbtcSocket.prototype.createFuturesOrder = async function(options) { // async
  const remaining=await this._bucket["wsFuturesOrder"].removeTokens(1);
...
HitbtcSocket.prototype.getFuturesAccounts = async function() { // async
  const remaining=await this._bucket["wsFuturesBalance"].removeTokens(1);
...
HitbtcSocket.prototype.subscribeTransactions = async function() {
  const remaining=await this._bucket["wsWalletSubscribe"].removeTokens(1);
