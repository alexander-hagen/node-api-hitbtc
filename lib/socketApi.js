...
module.exports = {
  ...
  accountApi: function(keys) { return new HitbtcSocket(accountUrl, keys); }
};
...
//
// ACCOUNT Socket Functions
//

HitbtcSocket.prototype.subscribeTransactions = function() {
  this.socket.request('subscribeTransactions', {});
};

HitbtcSocket.prototype.unsubscribeTransactions = function() {
  this.socket.request('unsubscribeTransactions', {});
};

HitbtcSocket.prototype.subscribeBalance = function() {
  this.socket.request('subscribeBalance', {});
};

HitbtcSocket.prototype.unsubscribeBalance = function() {
  this.socket.request('unsubscribeBalance', {});
};

HitbtcSocket.prototype.getBalance = async function() { // sync
  try {
    const result = await this.socket.request('getBalance', { });
    return result;
  }
  catch(e) {
...
HitbtcSocket.prototype.findTransactions = async function(options) {
  try {
    const result = await this.socket.request('findTransactions', options);
    return result;
  }
  catch(e) {
    logger.error('findTransactions failed', e);
    return null;
  };
};

HitbtcSocket.prototype.loadTransactions = async function(options) {
  try {
    const result = await this.socket.request('loadTransactions', options);
    return result;
  }
  catch(e) {
    logger.error('findTransactions failed', e);
