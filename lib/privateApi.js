...
var HitbtcPrivate = function(api) {
  ...
  this._bucket={
    subAccount: new TokenBucket({ bucketSize: 50, tokensPerInterval: 20, interval: "second" }),
  };
...
//!Object.keys(options.data).length?"":JSON.stringify(options.data))+
  ...
  options["headers"]={'Authorization': 'HS256 '+Buffer.from(this.apikey+":"+signature+":"+stamp+":"+window).toString('base64') };
...
HitbtcPrivate.prototype.getSpotOrders = async function(options={}) {
  ...
  return await new Promise(async (resolve, reject) => {
    const remaining=await bucket["spot"].removeTokens(1);
    const output=await this.getQuery(path,{});
    if(output.hasOwnProperty("error")) { reject(output); }
    else { resolve(output); };
  });
...
HitbtcPrivate.prototype.getMarginAccounts = async function() {
  ...
  return await new Promise(async (resolve, reject) => {
    const remaining=await bucket["margin"].removeTokens(1);
    const output=await this.getQuery(path,{});
    if(output.hasOwnProperty("error")) { reject(output); }
    else { resolve(output); };
  });
...
HitbtcPrivate.prototype.getFuturesAccounts = async function() {
  ...
  return await new Promise(async (resolve, reject) => {
    const remaining=await bucket["futures"].removeTokens(1);
    const output=await this.getQuery(path,{});
    if(output.hasOwnProperty("error")) { reject(output); }
    else { resolve(output); };
  });
...
HitbtcPrivate.prototype.getWalletBalance = async function() {
  ...
  return await new Promise(async (resolve, reject) => {
    const remaining=await bucket["wallet"].removeTokens(1);
    const output=await this.getQuery(path,{});
    if(output.hasOwnProperty("error")) { reject(output); }
    else { resolve(output); };
  });
...
HitbtcPrivate.prototype.getSubaccounts = async function() {
  ...
  return await new Promise(async (resolve, reject) => {
    const remaining=await this._bucket["subAccount"].removeTokens(1);
    const output=await this.getQuery(path,{});
    if(output.hasOwnProperty("error")) { reject(output); }
    else { resolve(output); };
  });
