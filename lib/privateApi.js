...
//options["headers"]={'Authorization': 'Basic '+Buffer.from(this.apikey+":"+this.secret).toString('base64') };
  ...
  try {
    ...
    if(!err.hasOwnProperty("response")) { Object.assign(response,{ status: "503", code: err.code }); }
      ...
      Object.assign(response,{ status: err.status, code: err.response.status });
      if(err.response.hasOwnProperty("data")) { Object.assign(response,{ error: err.response.data.error.code, reason: err.response.data.error.message }); };
    };
    return response;
