...
//  return new HitbtcSocketPublic(publicUrl, keys);
//};

module.exports = {
  publicApi: function() { console.log("1:"+arguments.callee.name);return new HitbtcSocketPublic(publicUrl); },
...
HitbtcSocketTrading.prototype.getTradingBalance = async function() { // sync
  try {
    ...
    logger.warn('Error caught (getTradingBalance):', e);
//    process.exit(0);
    return [];
  };
};
...
HitbtcSocketTrading.prototype.getOrders = async function() { // sync
  try {
    ...
    logger.warn('Error caught (getOrders):', e);
//    process.exit(0);
    return [];
  };
};
...
HitbtcSocketTrading.prototype.newOrder = async function(symbol, price, quantity, side, type, stopPrice, order_id) { // async
  ...
  try {
    ...
    switch(e.code) {
      case 20001:
      case 20008: logger.warn('Error caught (newOrder):', e); break;
      default: break;
    };
  };
...
HitbtcSocketTrading.prototype.cancelOrder = async function(clientOrderId) { // async
  ...
  try {
    ...
    switch(e.code) {
      case 20001:
      case 20008:
      default: logger.warn('Error caught (cancelOrder):',e);break;
    };
  };
...
HitbtcSocketTrading.prototype.replaceOrder = async function(clientOrderId, newOrderId, price, quantity) { // async
  ...
  try {
    ...
    switch(e.code) {
      case 20001:
      case 20008: logger.warn('Error caught (replaceOrder):',e);break;
      default: break;
    };
  };
